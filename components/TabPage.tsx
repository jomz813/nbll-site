...
const westernStandings = [
  { rank: 1, team: 'Los Angeles Clippers', w: 9, l: 0, gb: '0' },
  { rank: 2, team: 'Dallas Mavericks', w: 7, l: 2, gb: '2.0' },
  { rank: 3, team: 'Denver Nuggets', w: 5, l: 4, gb: '4.0' },
  { rank: 4, team: 'Los Angeles Lakers', w: 4, l: 5, gb: '5.0' },
  { rank: 5, team: 'Memphis Grizzlies', w: 2, l: 6, gb: '6.5' },
  { rank: 6, team: 'Minnesota Timberwolves', w: 1, l: 7, gb: '7.5' },
  { rank: 7, team: 'Oklahoma City Thunder', w: 0, l: 9, gb: '9.0' }
];

const easternStandings = [
  { rank: 1, team: 'Chicago Bulls', w: 6, l: 2, gb: '0' },
  { rank: 2, team: 'Milwaukee Bucks', w: 7, l: 3, gb: '0' },
  { rank: 3, team: 'Toronto Raptors', w: 6, l: 3, gb: '0.5' },
  { rank: 4, team: 'Orlando Magic', w: 5, l: 3, gb: '1.0' },
  { rank: 5, team: 'Boston Celtics', w: 4, l: 5, gb: '2.5' },
  { rank: 6, team: 'Miami Heat', w: 3, l: 5, gb: '3.0' },
  { rank: 7, team: 'Atlanta Hawks', w: 2, l: 7, gb: '4.5' }
];
...
const TabPage: React.FC<TabPageProps> = ({ tabId, onBack, onTabChange }) => {
  const [openWeeks, setOpenWeeks] = useState<number[]>([5]);
  ...
  const HOFEligibility: React.FC = () => {
    ...
    return (
      <div className="w-full bg-zinc-50/50 border border-zinc-100 rounded-2xl overflow-hidden transition-colors hover:bg-zinc-50">
        <button 
          onClick={() => setExpanded(!expanded)}
          className="w-full px-6 py-4 flex items-center justify-between text-left outline-none group"
        >
          <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] group-hover:text-zinc-500 transition-colors">Eligibility Requirements</span>
          <svg 
            className={`w-4 h-4 text-zinc-300 transition-transform duration-300 ${expanded ? 'rotate-180' : ''} group-hover:text-zinc-400`} 
            viewBox="0 0 24 24" 
            fill="none" 
            stroke="currentColor" 
            strokeWidth="2" 
        ...
        <div className={`transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden ${expanded ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="px-6 pb-6 border-t border-zinc-100/50">
            <ul className="space-y-3 pt-2">
              {[
                '1x Championship Ring',
                '2x Finals Appearances',
                '25x Player of the Games + Defensive Player of the Games',
                '4x Seasons Played',
                '5x Awards',
                'Ring Riding does not count'
              ].map((item, idx) => (
                <li key={idx} className="flex items-center gap-3 text-xs font-bold text-zinc-500 uppercase tracking-wide">
                ...
                {hallOfFameMembers.map((member, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {/* Front Face */}
                        <div className="absolute inset-0 backface-hidden bg-white border-2 border-zinc-100 rounded-[2rem] overflow-hidden flex flex-col h-full">
                           ...
                           <div className="p-6 lg:p-5 bg-white border-t border-zinc-50 flex flex-col flex-1">
                              <div className="space-y-4 lg:space-y-2 flex-1">
                                 <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black uppercase tracking-[0.25em] text-zinc-300">About</span>
                                    <div className="w-1.5 h-1.5 rounded-full bg-zinc-200" />
                                 </div>
                                 <p className="text-xs font-bold text-zinc-400 italic leading-relaxed lg:line-clamp-2 overflow-hidden">
                                   Description coming soon.
                                 </p>
                              </div>
                           </div>
                           <div className="absolute bottom-3 right-4 opacity-0 group-hover:opacity-100 transition-opacity text-[9px] font-black text-zinc-300 uppercase tracking-widest pointer-events-none">
                             tap to flip
                           </div>
                        </div>
